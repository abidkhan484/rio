...
- Support python 3.14
- Avoid adding a stack frame to user crashes
- add workaround for Tabs not updating its content in some cases
- Tables can scroll, `backend` parameter
- more defensive data ingestion for tables
- NumberInput now increments/decrements its value on uparrow/downarrow press
- dev tools improvements
- fix `Text` ignoring strikethrough and underline from its `TextStyle`
- fix: restrict allowed URL schemes to http and https
- fix: improve path safety in icon registry
- fix Cards not reacting to corner radius changes
- fix crash caused by incorrect import
- fix image not using all available space while loading
- fix MediaPlayer reporting error on destruction
- fix matplotlib plots being too large
